...
const context = (config = defaultContextOptions()) => {
  const { excludeExtension, includeExtension, excludeName, includeName } = Object.assign(defaultContextOptions(), config);
  const files         = require.context('./', true, /\.(.[^./])+$/),
        excludeFilter = filterRegular(orRegular(excludeName), orRegular(excludeExtension)),
